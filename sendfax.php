...
if(!isset($_POST["dest"]) || $_POST["dest"] == "" || !preg_match("/^[0-9]+$/", $_POST["dest"])) {
	errMsg(_("Numero di telefono non valido"), "index.php");
}

if(!isset($_FILES["f"]["name"]) || $_FILES["f"]["name"] == "") {
    errMsg(_("Documento FAX non selezionato"), "index.php");
}
...
if($mime != "application/pdf") {
    errMsg(_("Il documento FAX selezionato deve essere di tipo PDF"), "index.php");
}
...
if($ret != 0) {
    errMsg(_("FAX non inviato. Errore interno durante l'invio del FAX."), "index.php");
}
...
if($ret != 0) {
    ...
    infoMsg(_("FAX accodato per l'invio."), "index.php");
}
